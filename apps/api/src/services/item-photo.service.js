...
import { uploadFile, deleteFile, createSignedUrl, } from "./file-storage.service.js";
async function getOrderItemForBusiness(businessId, orderId, orderItemId) {
...
export async function createItemPhoto(businessId, orderId, orderItemId, input) {
    const orderItem = await getOrderItemForBusiness(businessId, orderId, orderItemId);
    if (!input.file || input.file.length === 0) {
        throw new Error("File foto wajib diisi");
    }
    const extension = getExtension(input.contentType, input.originalName);
    const randomId = crypto.randomUUID();
    const storageKey = `businesses/${businessId}` +
        `/orders/${orderId}` +
        `/items/${orderItem.id}` +
        `/${randomId}${extension}`;
    await uploadFile(input.file, storageKey, input.contentType);
    try {
        ...
        try {
            ...
            console.error("Gagal cleanup file Supabase:", cleanupError);
        }
        throw error;
    }
}
export async function getItemPhotos(businessId, orderId, orderItemId) {
    await getOrderItemForBusiness(businessId, orderId, orderItemId);
    ...
    const result = await Promise.all(photos.map(async (photo) => {
        ...
        const signedUrl = await createSignedUrl(photo.storageKey, 60 * 15);
        return {
            ...photo,
            url: signedUrl,
        };
...
export async function deleteItemPhoto(businessId, orderId, orderItemId, photoId) {
    await getOrderItemForBusiness(businessId, orderId, orderItemId);
...
//   });
// }
//# sourceMappingURL=item-photo.service.js.map