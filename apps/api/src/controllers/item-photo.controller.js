import { createItemPhoto, getItemPhotos, deleteItemPhoto, } from "../services/item-photo.service.js";
export async function create(req, res) {
    try {
        ...
        const photo = await createItemPhoto(businessId, orderId, itemId, {
            file: file.buffer,
        ...
        console.error("Create item photo error:", error);
        return res.status(400).json({
            success: false,
...
export async function list(req, res) {
    try {
        ...
        const photos = await getItemPhotos(businessId, orderId, itemId);
        return res.json({
            success: true,
            data: photos,
        });
...
export async function remove(req, res) {
    try {
        const businessId = req.user.businessId;
        const { orderId, itemId, photoId, } = req.params;
        if (typeof orderId !== "string" ||
            typeof itemId !== "string" ||
        ...
        await deleteItemPhoto(businessId, orderId, itemId, photoId);
        return res.json({
            success: true,
...
//   }
// }
//# sourceMappingURL=item-photo.controller.js.map